...
const CYAN = "#16C1F3";
const WHITE = "#FFFFFF";
const MUTED = "rgba(214,228,236,.72)";
...
export default function UpcomingEvents() {
  ...
  return (
    <section id="events" className="relative overflow-hidden px-6 py-20 md:px-10 md:py-24 lg:px-[55px] lg:py-[120px]" style={{ background: "#071F2D" }}>
      <div aria-hidden="true" className="pointer-events-none absolute inset-0" style={{ background: "radial-gradient(circle at 9% 12%, rgba(22,193,243,.13), transparent 28%), radial-gradient(circle at 94% 88%, rgba(10,106,136,.18), transparent 30%)" }} />
      <div aria-hidden="true" className="pointer-events-none absolute inset-y-0 left-[42%] hidden w-px lg:block" style={{ background: "linear-gradient(180deg, transparent, rgba(255,255,255,.12) 18%, rgba(255,255,255,.12) 82%, transparent)" }} />
      ...
      <div className="relative z-[1] grid gap-12 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)] lg:gap-20">
        <div className="flex flex-col items-start gap-6 lg:sticky lg:top-24 lg:self-start">
          ...
          <h2 className="t-h2 m-0 max-w-[650px]" style={{ color: WHITE }}>
            Meet MQS at the next <span style={{ color: CYAN }}>industry exhibition</span>.
          </h2>
          ...
          <div className="t-caption inline-flex items-center gap-2 border px-3 py-2" style={{ color: "rgba(255,255,255,.78)", borderColor: HAIRLINE, background: "rgba(255,255,255,.035)" }}>
          ...
          <div className="mt-2 flex flex-wrap gap-3">
            ...
            <a href="/products" className="t-button inline-flex h-12 items-center border px-6 no-underline transition-colors hover:bg-white/10" style={{ color: WHITE, borderColor: "rgba(255,255,255,.22)" }}>
              See our products
              <span aria-hidden="true" className="ml-3">→</span>
            </a>
          </div>
        ...
        <div>
          <div className="mb-5 flex items-end justify-between gap-4">
            ...
            <div className="t-caption whitespace-nowrap" style={{ color: "rgba(255,255,255,.54)" }}>04 events</div>
          </div>

          <div className="flex flex-col gap-4">
            {visibleEvents.map((item, index) => (
              <article key={item.event} className="group relative grid overflow-hidden border transition duration-300 hover:-translate-y-0.5 hover:bg-white/[.075] sm:grid-cols-[150px_1fr]" style={{ borderColor: HAIRLINE, background: "rgba(255,255,255,.045)" }}>
                ...
                <div className="flex flex-row items-center justify-between gap-5 border-b px-5 py-5 sm:flex-col sm:items-start sm:justify-center sm:border-b-0 sm:border-r sm:px-6" style={{ borderColor: HAIRLINE }}>
                  <span className="t-caption" style={{ color: index === 0 ? CYAN : "rgba(255,255,255,.46)" }}>0{index + 1}</span>
                  <p className="t-body m-0 font-semibold" style={{ color: WHITE }}>{item.dates}</p>
                </div>
                <div className="flex flex-col gap-4 px-5 py-6 sm:px-7">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <h3 className="t-h4 m-0" style={{ color: WHITE }}>{item.event}</h3>
                    <span className="t-caption border px-3 py-1.5" style={{ color: "rgba(255,255,255,.78)", borderColor: HAIRLINE, background: "rgba(255,255,255,.035)" }}>{item.city}</span>
                  </div>
                  <div>
                    <div className="t-caption mb-1.5" style={{ color: CYAN }}>On display</div>
          ...
          <p className="t-caption m-0 mt-7" style={{ color: "rgba(255,255,255,.48)" }}>Confirmed MQS participation · All dates 2026</p>
        </div>
      </div>
    </section>
  );
